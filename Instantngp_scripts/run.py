...
def parse_args():
	...
	parser.add_argument("--save_metrics", default="", help="File with path to save PSNR and metrics.")
	...
	parser.add_argument("--screenshot_depth", action="store_true", help="Render depth instead of RGB")
...
if __name__ == "__main__":
	...
	if ref_transforms:
		...
		for idx in args.screenshot_frames:
			...
			if args.screenshot_depth:
				write_image_depth(outname, image, ref_transforms["scale"])
			else:
	...
	if args.video_camera_path:
		...
		for i in tqdm(list(range(min(n_frames, n_frames+1))), unit="frames", desc=f"Rendering video"):
			...
			if save_frames:
				...
				write_image("ftmp/{i:04d}.jpg", np.clip(frame * 2**args.exposure, 0.0, 1.0), quality=100)
